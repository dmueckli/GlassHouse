...
class GlassHouseException : virtual public std::exception
{
...
public:
     ...
     */
    explicit GlassHouseException(int err_num, const String &msg) : error_number(err_num), error_message(msg)
    {
    }
     ...
     */
    virtual ~GlassHouseException() throw() {}
...
class GlassHouseMk1 : virtual public GlassHouse
{
private:
    int _statusCode;
    int _hostId;
...
public:
    ...
    static String createJsonMessage()
    {
        ...
        jsonMessage["sensor data"]["humidity"] = airHumiditySensor.getHumidity();
        jsonMessage["sensor data"]["soil moisture"] = moistureSensor.getMoistureValue(true);
        jsonMessage["sensor data"]["temperature °C"] = airHumiditySensor.getTemperature();
        jsonMessage["sensor data"]["heat index °C"] = airHumiditySensor.getHeatIndex();
        jsonMessage["sensor data"]["temperature °F"] = airHumiditySensor.getTemperature(true);
        jsonMessage["sensor data"]["heat index °F"] = airHumiditySensor.getHeatIndex(true);
